...
class BlockchainCredentialVault {
  constructor() {
    ...
    this.contractABI = [
      {
        inputs: [
          { name: '_credentialHash', type: 'string' },
  ...
  async verifyCredential(credentialHash) {
    try {
      ...
      const verificationResult = await this.simulateBlockchainQuery('verifyCredential', [
        credentialHash,
      ]);
      ...
      const nftMetadata = {
        ...
        attributes: [
          ...
          { trait_type: 'Issue Date', value: new Date(credentialData.issueDate).toISOString() },
          {
            ...
            value: this.getVerificationLevelName(credentialData.verificationLevel),
          },
  ...
  async submitToEmbassyVerification(credentialHash, embassyCode) {
    try {
      console.log(
        `🏛️ Submitting credential ${credentialHash} to embassy ${embassyCode} for verification`,
      );
  ...
  async simulateEmbassyResponse(credentialHash, embassyCode) {
    ...
    window.dispatchEvent(
      new CustomEvent('embassyVerificationComplete', {
        detail: response,
      }),
    );
  ...
  async getUserCredentials(userAddress) {
    ...
    const userCredentials = cached.filter(
      (cred) => cred.metadata && cred.metadata.did === this.generateDID(address),
    );
  ...
  try {
    ...
    if (!window.Web3) {
      ...
      script.onload = async () => {
        ...
        window.dispatchEvent(
          new CustomEvent('blockchainCredentialsReady', {
