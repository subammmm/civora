...
class ScholarshipMatcher {
  ...
  generateMatchReasons(scholarship) {
    ...
    if (profile.targetDegree && scholarship.level.toLowerCase().includes(profile.targetDegree.toLowerCase())) {
      reasons.push(`Matches your ${profile.targetDegree} degree goal`);
    }
    
    if (profile.fieldOfStudy && scholarship.fields && scholarship.fields.includes(profile.fieldOfStudy)) {
  ...
  generateApplicationTips(scholarship, matchScore) {
    ...
    if (!profile.workExperience || parseInt(profile.workExperience) === 0) {
      tips.push('Consider gaining relevant work or volunteer experience to strengthen your profile.');
    }
